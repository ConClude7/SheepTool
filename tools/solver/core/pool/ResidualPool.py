...
class ResidualPool(object):
    def __init__(self, card_container):
        ...
        # 当前操作池卡牌类别及其卡牌序号
        self._pool_card = []
        # card_type → card_pair 的哈希索引，_find_match_card_pair O(k)→O(1)
        self._pool_card_map = {}
        # 当前操作池消除记录及其初始位置
    ...
    def _create_card_pair(self, card_detail):
        ...
        self._pool_card_map[card_type] = card_pair
        return card_pair

    def _recover_card_pair(self, card_pair):
        self._pool_card.remove(card_pair)
        del self._pool_card_map[card_pair["card_type"]]
    ...
    def _recover_disappear_item(self, card_detail):
        ...
        self._pool_card_map[card_pair["card_type"]] = card_pair
        self._pool_count += 3
        return card_pair
    ...
    def _find_match_card_pair(self, card_detail):
        return self._pool_card_map.get(card_detail.get_card_type())
