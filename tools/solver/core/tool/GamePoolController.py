...
class GamePoolController(object):
    ...
    def _get_cached_main_zone_list(self):
        if self._main_zone_cache is None:
            self._main_zone_cache = self._operation_pool.get_main_zone_show_card_list(self._solver_algorithm)
        return self._main_zone_cache

    def check_game_over(self):
        return len(self._get_cached_main_zone_list()) == 0

    def check_fingerprint_exist(self):
        fingerprint = frozenset(self._get_cached_main_zone_list())
    ...
    def generate_head_list(self):
        return self._get_cached_main_zone_list()
    ...
    def _sort_head_list_with_type_list(self, card_type_dict, card_type_list):
        # 一次遍历建哈希分组，O(n+m) 替代原来的 O(m×n)
        type_to_indices = {}
        for index, card_type in card_type_dict.items():
            type_to_indices.setdefault(card_type, []).append(index)
        result_list = []
        for type_item in card_type_list:
            result_list.extend(type_to_indices.get(type_item, []))
        result_set = set(result_list)
        last_index_list = [index for index in card_type_dict.keys() if index not in result_set]
