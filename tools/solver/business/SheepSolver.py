...
class SheepSolver(object):
    def __init__(self, solver_config, solver_algorithm):
        ...
        self._card_count = 0
        self._current_pick_count = 0
        self._current_progress = 0
        self._maximum_progress = 0
        self._best_partial_index_list = []
        self._partial_accept_reached = False
        self._last_progress_emit_wall_time = 0.0

    def _prepare_runtime_param(self, solver_config, solver_algorithm):
        self._solver_config = solver_config
        self._partial_accept = solver_config.get("partial_accept", 0.0)
        self._progress_queue = solver_config.get("progress_queue")
        self._progress_label = solver_config.get("progress_label", solver_algorithm)
        self._progress_emit_interval = solver_config.get("progress_emit_interval", 0.2)
    ...
    def solve(self):
        ...
        if self._partial_accept_reached:
            return
    ...
    def _show_solving_progress(self):
        ...
        if self._solver_config["show_progress"] and self._verify_within_time_period(1):
            content = "当前进度为: {}/{}".format(self._current_pick_count, self._card_count)
    ...
    def _emit_progress(self, force=False):
        if self._progress_queue is None or self._card_count <= 0:
            return

        now = time.time()
        if not force and now - self._last_progress_emit_wall_time < self._progress_emit_interval:
            return
    ...
    def emit_progress_snapshot(self, force=False):
    ...
    def get_maximum_progress(self):
        return self._maximum_progress

    def generate_best_partial_card_id_result(self):
        """返回进度最高时的 card_id 序列（不要求完整解）。"""
        if not self._best_partial_index_list:
            return None
        card_detail_dict = self._generate_card_detail_dict()
        return self._generate_card_id_list(card_detail_dict, self._best_partial_index_list)
